...
def addAppointment(request):
    ...
    try:
        ...
        data_upload={"compare_date": (datetime.date.today()).strftime('%m/%d/%Y'),}
    ...
    if request.is_ajax():
        if request.method == 'POST':
            ...
            book_day = datetime.datetime.strptime(book_date, '%m/%d/%Y').date().strftime('%A')
            ...
            if check_week_day or check_holiday:
                ...
                book_date = datetime.datetime.strptime(book_date, '%m/%d/%Y').date()
                ...
                if book_date <= end_date and cmp_date >= end_date:
                    ...
                    with open(json_dir) as f:
                        data = json.load(f)
                        compare_date = data.get("compare_date")
                    compare_date=datetime.datetime.strptime(compare_date, '%m/%d/%Y').date()
